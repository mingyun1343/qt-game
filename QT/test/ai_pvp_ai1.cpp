...
int pvp_ai1_AimBall(DataStruct *data, BallStruct &ball, int &leftward, int &rightward)
{
    ...
    float ball_v=pvp_ai1_vecMod(ball_v_x,ball_v_y);

    double distance_x=fabs(data->plane1.x-ball_x);
    double distance_y=fabs(data->plane1.y-ball_y);
    double distance=pvp_ai1_vecMod(distance_x,distance_y);
    ...
    if(t1>=0){t=t1;}
        ...
        angleTo=pvp_ai1_angleConver(atan2(yy,xx)*180/PI);
...
int pvp_ai1_aimplane2(DataStruct *data,int &leftward,int &rightward){
    double x=data->plane2.x;
    double y=data->plane2.y;
    double angleTo;
    double angleDiff;
    int flag=0;
    angleTo =
       pvp_ai1_angleConver(atan2(y - data->plane1.y, x - data->plane1.x) * 180 / PI);
    angleDiff=data->plane1.angle-angleTo;
...
int pvp_ai1_cho_ball(DATASTRUCT *data){
    int max=0;
...
bool pvp_ai1_aroundDanger(DATASTRUCT*data){
    for(int i=0;i<data->ball_size;i++){
        double distance_x=fabs(data->plane1.x-data->ball[i].x);
        double distance_y=fabs(data->plane1.y-data->ball[i].y);
        double distance=pvp_ai1_vecMod(distance_x,distance_y);
...
void ai_pvp_ai1(DataStruct*data,KeyStruct*key){
 ...
 //   qDebug()<<time_counter;
    ...
    if (pvp_ai1_moveNeeded(data, des_x, des_y)&&pvp_ai1_aroundDanger(data)){
        key->shield=true;
    }

    //如果球个数不为零
    if (data->ball_size&&(time_counter%600)>80)
    {
        int max=0;
        max=pvp_ai1_cho_ball(data);

        pvp_ai1_AimBall(data, data->ball[max], key->rotate_left, key->rotate_right);
    }
    else{
        pvp_ai1_aimplane2(data,key->rotate_left, key->rotate_right);
    }
}
